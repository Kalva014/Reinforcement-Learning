...
def get_action_egreedy(values, epsilon, state):
	# Implement epsilon greedy action policy
	if np.random.random() < epsilon: # With probability !, perform a randomly selected action
		action = np.random.randint(4) # 4 actions
	else: # With probability 1 − !, perform a greedy action
